 ...
 * @brief Main program for the optimized heat diffusion simulation
 */
...
#include <algorithm> // For std::min_element and std::max_element
 ...
 void printUsage(const char* programName) {
     std::cout << "Usage: " << programName << " [OPTIONS]\n"
               ...
               << "  --output-dir DIR       Directory for output files (default: output/optimised)\n"
               << std::endl;
 }
 
 /**
  * @brief Main function running the optimized heat diffusion simulation
  */
 int main(int argc, char* argv[]) {
     ...
     for (int run = 0; run < numRuns; run++) {
        ...
        OptimizedHeatDiffusion simulation(width, height, diffusionRate, saveOutput);
