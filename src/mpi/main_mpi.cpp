...
void runSimulation(int width, int height, double diffusionRate, int totalFrames, bool saveOutput, bool visualizeHalos);
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        for (int i = 1; i < argc; i++) {
            ...
            if (arg == "--help") {
                ...
                saveOutput = true;
            } else if (arg == "--visualize-halos") {
                visualizeHalos = true;
                saveOutput = true;  // Halo visualization requires output to be enabled
    ...
    MPI_Bcast(&visualizeHalos, 1, MPI_C_BOOL, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        std::cout << "Running MPI-parallelized heat diffusion simulation with parameters:" << std::endl
                  ...
                  << "  Halo visualization: " << (visualizeHalos ? "enabled" : "disabled") << std::endl;
    }
    ...
    // Run the simulation
    runSimulation(width, height, diffusionRate, totalFrames, saveOutput, visualizeHalos);
...
void runSimulation(int width, int height, double diffusionRate, int totalFrames, bool saveOutput, bool visualizeHalos) {
    ...
    for (int run = 0; run < numRuns; run++) {
        ...
        // Create simulation instance with proper settings
        OptimizedHeatDiffusionMPI simulation(width, height, diffusionRate, saveOutput);
        
        // If this is a visualization run, save initial state with halos
        if (visualizeHalos && saveOutput && run == 0) {
            simulation.saveHaloVisualization(0);
        }
...
void printUsage(const char* programName) {
    std::cout << "Usage: " << programName << " [OPTIONS]\n"
              ...
              << "  --frames FRAMES        Set number of frames to simulate (default: 1000)\n"
              << "  --output               Enable output file generation\n"
              << "  --visualize-halos      Enable halo region visualization (implies --output)\n"
